...
var myAtoi = function (str) {
    ...
    str = str.trim();
    //* Check sign, the first char has to be '-' or '+', if not while loop runs, by default set to 1(positive).
    if (str[i] == '-' || str[i] == '+') sign = str[i++] == '-' ? -1 : 1;
    //* check if the char is a number by checking char code using 'char'.charCodeAt(0).
    while (str[i] && str[i].charCodeAt(0) - 48 <= 9 && str[i].charCodeAt(0) - 48 >= 0) {
        //* Add on the the num, increment i by 1, and num by *10.
